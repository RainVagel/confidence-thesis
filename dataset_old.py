    ...
    def __init__(self, batch_size, augm_flag, test_only=True):
        ...
        if test_only:
            ...
            self.test_dataset = torch.utils.data.Subset(
                datasets.LSUN(self.data_dir, classes=['classroom_train'], transform=transform_test), range(0, 10000))
...
class ImageNetMinusCifar10(Dataset):
    def __init__(self, batch_size, augm_flag):
        ...
        self.test_dataset = torch.utils.data.Subset(datasets.ImageFolder(self.test_dir, transform=transform_test),
                                                    np.random.permutation(range(self.n_test))[:10000])
